...
export async function generateMetadata({
  ...
  if (!category) return { title: "Category Not Found | Patel Global Green" };

  return {
    title: `${category.name} | Buy Online in Delhi NCR | Patel Global Green`,
    description: `Explore our collection of ${category.name} with free next-day delivery across Delhi NCR. ${category.description}`,
    openGraph: {
      title: `${category.name} - Patel Global Green Nursery`,
...
export default async function CategoryPage({ params }: CategoryPageProps) {
  ...
  const categoryPlants = plants.filter((p) => p.category === category.id);
          ...
          <div className="row align-items-center g-4">
            <div className="col-lg-7">
              ...
              <p className="lead text-muted mb-4" style={{ fontSize: "1.15rem" }}>
                {category.description}. Nursery-grown with 100% health guarantee and next-day free delivery across Delhi NCR.
              </p>
